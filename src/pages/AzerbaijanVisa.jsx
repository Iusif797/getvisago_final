...
const AzerbaijanVisa = () => {
  ...
  const desktopVisaData = {
    ...
    getVisaLink: "/get-visa/azerbaijan",
    agencyText: `GetVisaGo Ltd — your independent private agency for fast and secure Azerbaijan e-Visa processing. Apply fully online in minutes, without embassy visits or paperwork hassle. Our single transparent fee covers both our professional service (document check, secure submission, and 24/7 support) and the official government visa cost — no extra payments required. We are not affiliated with the Azerbaijan government; we assist you to get your visa easily and stress-free.`,
    visaTypes: [
      { title: 'single-entry', icon: touristIcon, applyLink: "/get-visa/azerbaijan?type=single-entry", details: [{ label: 'Validity', value: '90 days' }, { label: 'Stay per visit', value: '30 days' }, { label: 'Government fee', value: '₹1720' }, { label: 'Service fee', value: '₹2590' }] },
      { title: 'Business eVisa', icon: touristIcon, applyLink: "/get-visa/azerbaijan?type=Business+eVisa", details: [{ label: 'Validity', value: '60 days' }, { label: 'Stay per visit', value: '30 days' }, { label: 'Government fee', value: '13017,32 ₹' }, { label: 'Service fee', value: '2590 ₹' }] },
    ]
  };

  const [currentSlide, setCurrentSlide] = useState(0);
  const slides = [
    { id: 1, image: bannerSA1, title: "Discover the Land of Fire, where ancient history meets modern architecture.", date: "31 May, 2025" },
    { id: 2, image: bannerSA2, title: "From the Caspian Sea to the Caucasus Mountains, Azerbaijan is a land of contrasts.", date: "31 May, 2025" }
  ];
  ...
  const visaTypes = [
    { title: 'single-entry', validity: '90 days', stay: '30 days', govFee: '₹1720', serviceFee: '₹2590' },
    { title: 'Business eVisa', validity: '60 days', stay: '30 days', govFee: '13017,32 ₹', serviceFee: '2590 ₹' }
  ];
          ...
          <div className="relative px-4 sm:px-6 lg:px-8 -mt-32 sm:-mt-40">
            <div className="bg-white rounded-2xl shadow-lg p-6 w-full max-w-md mx-auto flex flex-col">
              <h1 className="text-3xl lg:text-4xl font-bold mb-6 text-center">
                ...
                <span className="bg-gradient-to-r from-[#8B5CF6] to-[#3B82F6] bg-clip-text text-transparent">Azerbaijan</span>
              </h1>
              <div className="space-y-4 mb-6">
                ...
                <div className="bg-white border border-gray-100 rounded-xl shadow-sm p-3 text-center w-full">
                  ...
                  <p className="text-gray-900 font-bold text-base">From 4239 ₹</p>
                </div>
              </div>
              <Link
                to="/get-visa/azerbaijan"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {visaTypes.map((visa, index) => (
                <div key={index} className="bg-white rounded-2xl shadow-lg p-6">
                  ...
                  <Link to={`/get-visa/azerbaijan?type=${encodeURIComponent(visa.title)}`} className="block w-full mt-6 bg-white border-2 border-purple-600 text-purple-600 py-3 rounded-full font-bold text-center text-lg hover:bg-purple-50 transition-all duration-300">
                    APPLY NOW
                  </Link>
                </div>
              ))}
            ...
            {slides.map((slide, index) => (
              <div key={slide.id} className={`transition-all duration-500 ${isDesktop || index === currentSlide ? 'block' : 'hidden'}`}>
                <div className="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-all duration-300">
                  <div className="relative">
                    <img src={slide.image} alt={`Azerbaijan - ${slide.id}`} className="w-full h-64 object-cover" />
                  </div>
                  <div className="p-6">
